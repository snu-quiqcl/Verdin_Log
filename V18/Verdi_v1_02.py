...
# This script is stored in O:\Device Manuals and Data\Coherent Mira HP-D and Verdi-V18\Monitoring Scripts\python3.5\MiraControl\V18

# Change log
# v1_00: Initial version
# v1_01: Added getCurrentFault() method which provides the existing fault list. Different from the manual, the fault history won't be cleared until the controller is power-cycled.
...
# Monitoring status of the Verdi
    ...
    def getCurrentFault(self):
        """ Returns the current fault. Each fault type can be found with
        faultStatusMsg dictionary. When there is no fault, empty list
        will be returned.
        ...
        if response == 'SYSTEM OK':
            ...
            faultList = map(int, response.split('&'))
            """
            According to the manual, the list will be separated by &, but it was
            not correct. Those numbers were separated by comma, and '?F'
            returned string list separated by &. See the following:
            - '?F' returns '4&6&7&10&44&48'
            - '?FH' returns '48,10,4,7,6,44'
